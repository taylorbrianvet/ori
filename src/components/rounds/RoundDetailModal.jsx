...
import { X, CheckCircle, XCircle, Loader2, Plus, Trash2, Upload, FileText } from "lucide-react";
...
const SURGERY_RESIDENTS = [
  ...
  "Tanya Ford", "Chris Larson", "Mei Zhang", "Robert Kim", "Anna Petrov"
];
const SURGERY_FACULTY = [
  "Brian Taylor", "Karen Wells", "Michael Rhodes", "Nancy Doyle", "Amanda Ross"
];

function JournalAssignmentRow({ assignment, index, surgeryResidents, onUpdate, onRemove, onUploadPDF }) {
  ...
  return (
    <div className="flex items-center gap-2 py-2 border-b border-white/8 last:border-0">
      ...
      >
        ...
        {surgeryResidents.map(r => <option key={r} value={r}>{r}</option>)}
      </select>
      <input
...
export default function RoundDetailModal({ round, onClose, onSaved, staffList = [] }) {
  ...
  const surgeryResidents = SURGERY_RESIDENTS;
  const surgeryFaculty = SURGERY_FACULTY;
  ...
  const updateAssignment = (i, k, v) => {
    const updated = assignments.map((a, idx) => idx === i ? { ...a, [k]: v } : a);
    set("journal_assignments", updated);
  };
  ...
  const handlePDFUpload = async (index, file, resident) => {
    // Store pending upload — process on save
    setPendingPDFUploads(prev => {
      const existing = prev.filter(p => p.index !== index);
      return [...existing, { index, file, resident }];
    });
    // Show placeholder
  ...
  const handleSave = async (newStatus) => {
    ...
    // Process any pending PDF uploads
    ...
    await base44.entities.EducationalRound.update(round.id, payload);
      ...
      <motion.div initial={{ y: 60, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 60, opacity: 0 }}
        ...
        <div className="px-5 py-4 border-b border-white/10 flex items-center justify-between sticky top-0 glass-panel z-10">
          <div>
            ...
            <p className="text-xs text-white/40 mt-0.5">{form.department} · {form.start_time || "07:00"} – {form.end_time || "08:00"}</p>
          </div>
        ...
        <div className="p-5 space-y-5">
            ...
            <div className="flex flex-wrap gap-2">
              {EVENT_TYPES.map(t => (
                <button key={t} onClick={() => set("event_type", t)}
                  className={`text-xs px-3 py-1.5 rounded-lg border transition-colors ${form.event_type === t ? "bg-white/18 border-white/30 text-white" : "bg-white/4 border-white/12 text-white/45 hover:bg-white/10"}`}>
                  {t}
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <input
              className="w-full px-3 py-2 rounded-xl bg-black/30 border border-white/20 text-sm text-white placeholder:text-white/25 focus:outline-none focus:border-white/35"
              placeholder={isJournalClub ? 'e.g. "Journal club on liver tumors"' : form.event_type === "Textbook Review" ? "e.g. TJ Ch 1 Inflammatory Response" : "Topic…"}
              value={form.topic || ""}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input type="time" className="w-full px-3 py-2 rounded-xl bg-black/30 border border-white/20 text-sm text-white focus:outline-none focus:border-white/35"
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5 mb-2">
              {surgeryFaculty.map(f => (
                <button key={f} onClick={() => set("clinician", form.clinician === f ? "" : f)}
                  className={`text-xs px-2.5 py-1 rounded-lg border transition-colors ${form.clinician === f ? "bg-white/18 border-white/30 text-white" : "bg-white/4 border-white/12 text-white/45 hover:bg-white/10"}`}>
                  {f}
                </button>
              ))}
              <button onClick={() => set("clinician", "other")}
                className={`text-xs px-2.5 py-1 rounded-lg border transition-colors ${form.clinician === "other" || (form.clinician && !surgeryFaculty.includes(form.clinician)) ? "bg-white/18 border-white/30 text-white" : "bg-white/4 border-white/12 text-white/45 hover:bg-white/10"}`}>
                Other
              </button>
            </div>
            {(form.clinician === "other" || (form.clinician && !surgeryFaculty.includes(form.clinician) && form.clinician !== "")) && (
              <input placeholder="Enter clinician name…"
                className="w-full px-3 py-2 rounded-xl bg-black/30 border border-white/20 text-sm text-white placeholder:text-white/25 focus:outline-none"
                value={surgeryFaculty.includes(form.clinician) ? "" : (form.clinician === "other" ? "" : form.clinician)}
          ...
          <div>
            ...
            <button onClick={() => set("presenters", [])}
              className={`text-xs px-3 py-1.5 rounded-lg border transition-colors mb-2 mr-2 ${(!form.presenters || form.presenters.length === 0) ? "bg-white/18 border-white/30 text-white" : "bg-white/4 border-white/12 text-white/45 hover:bg-white/10"}`}>
              Everyone
            </button>
            <div className="flex flex-wrap gap-1.5">
              {surgeryResidents.map(r => (
                <button key={r} onClick={() => toggleInArray("presenters", r)}
                  className={`text-xs px-2.5 py-1 rounded-lg border transition-colors ${(form.presenters || []).includes(r) ? "bg-white/18 border-white/30 text-white" : "bg-white/4 border-white/12 text-white/45 hover:bg-white/10"}`}>
                  {r.split(" ")[0]}
                </button>
              ))}
            </div>
          ...
          {/* Residents Present */}
          <div>
            <label className="text-[10px] text-white/40 uppercase tracking-wider font-semibold block mb-2">Residents Present</label>
            <div className="flex flex-wrap gap-1.5">
              {surgeryResidents.map(r => (
                <button key={r} onClick={() => toggleInArray("residents_present", r)}
                  className={`text-xs px-2.5 py-1 rounded-lg border transition-colors ${(form.residents_present || []).includes(r) ? "bg-green-500/20 border-green-400/30 text-green-200" : "bg-white/4 border-white/12 text-white/45 hover:bg-white/10"}`}>
                  {r.split(" ")[0]}
                </button>
              ))}
            </div>
          ...
          {/* Faculty Present */}
          <div>
            <label className="text-[10px] text-white/40 uppercase tracking-wider font-semibold block mb-2">Faculty Present</label>
            <div className="flex flex-wrap gap-1.5">
              {surgeryFaculty.map(f => (
                <button key={f} onClick={() => toggleInArray("faculty_present", f)}
                  className={`text-xs px-2.5 py-1 rounded-lg border transition-colors ${(form.faculty_present || []).includes(f) ? "bg-blue-500/20 border-blue-400/30 text-blue-200" : "bg-white/4 border-white/12 text-white/45 hover:bg-white/10"}`}>
                  {f.split(" ")[0]}
                </button>
              ))}
            </div>
          ...
          {isJournalClub && (
            <div>
                ...
                {assignments.length === 0 ? (
                  ...
                  assignments.map((a, i) => (
                    <JournalAssignmentRow key={i} assignment={a} index={i}
                      surgeryResidents={surgeryResidents}
              ...
              <p className="text-[10px] text-white/25 mt-1.5">Upload PDFs to automatically add them to Journal Club. All uploads are processed when you save.</p>
            </div>
          )}
          ...
          {form.status === "approved" && (
            <div className="rounded-xl bg-black/25 border border-white/10 p-3">
              ...
              <p className="text-xs text-white/70 font-mono leading-relaxed select-all">
                ...
                {(form.residents_present || []).length > 0 ? `\nResidents: ${form.residents_present.join(", ")}` : ""}
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
              ...
              <div className="flex gap-3">
                <button onClick={() => setConfirmCancel(false)} className="flex-1 py-2.5 rounded-xl text-xs text-white/50 hover:bg-white/10 transition-colors">
                  Keep
                </button>

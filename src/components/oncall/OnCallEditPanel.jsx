            ...
            <div className="mt-4 pt-4 border-t border-white/8">
              ...
              {["primary","secondary","tertiary"].map(slot => (
                <div key={slot} className="flex items-center gap-2 mb-2">
                  ...
                  <select
                    value={editForm[slot]}
                    onChange={e => setEditForm(f => ({ ...f, [slot]: e.target.value }))}
                    className="flex-1 bg-white/8 border border-white/12 rounded-xl px-3 py-1.5 text-xs text-white/85 focus:outline-none focus:border-white/25"
                  >
